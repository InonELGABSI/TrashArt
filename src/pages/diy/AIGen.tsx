...
export const AIGen = () => {
    ...
    return (
        <div className="w-full px-4 pb-12">
            <div className="max-w-7xl mx-auto rounded-[24px] p-8 md:p-12 shadow-sm border border-[#EAE8E1]">
                
                {/* Upload Area */}
                <div 
                    className={`w-full bg-[#EDE8DE] rounded-[16px] h-80 flex flex-col items-center justify-center cursor-pointer transition-all relative overflow-hidden ${!file ? 'hover:bg-[#e4e2db]' : ''}`}
                ...
                >
                    ...
                    {file ? (
                        ...
                        <>
                            ...
                            <p className="text-[20px] mb-2">גררו תמונה לכאן</p>
                            <p className="text-[20px] uppercase tracking-wider">JPG, PNG</p>
                        </>
                    )}
                </div>
                ...
                <div className="mt-10 flex flex-col items-center min-h-[100px]">
                    ...
                    {generated && (
                        <div className="w-full animate-fade-in flex flex-col items-center">
                            <div className="flex flex-col md:flex-row justify-center gap-8 mb-10 w-full max-w-4xl">
                                {[
                                    ...
                                    <div 
                                        key={idx} 
                                        className={`flex flex-col items-center group cursor-pointer transition-all duration-300 w-48 ${selectedOption?.label === opt.label ? 'scale-105' : 'hover:scale-105'}`}
                                        onClick={() => setSelectedOption(opt)}
                                    >
                                        <div className={`w-full aspect-square bg-white rounded-[16px] overflow-hidden shadow-md mb-4 border-2 transition-all border-[var(--color-black)]`}>
                                            <img src={opt.img} alt={opt.label} className="w-full h-full object-cover" />
                                        </div>
                                        <span className={`text-[32px] font-bold ${selectedOption?.label === opt.label ? 'text-[#1A1A2E]' : 'text-[#1A1A2E]'}`}>{opt.label}</span>
                                    </div>
                                ))}
                            </div>
                            ...
                            <div className="flex justify-center">
                                <button 
                                    onClick={handleReset}
                                    className="bg-[#1A1A2E] text-white px-10 py-3 rounded-xl text-[24px] hover:bg-[#2a2a40] transition-colors shadow-lg"
                                >
                                    טעינה מחדש
                                </button>
                            </div>
